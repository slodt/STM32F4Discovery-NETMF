...
namespace DemoDS1307
{
    public class DS1307
    {
        ...
        public bool Running()
        {
            var buffer = ReadRam(SecondsAddr);
            var halted = (buffer & 0x80) > 0; //CH bit is set
            return !halted;
        }

        public DateTime GetDateTime()
        {
            var buffer = ReadRam(SecondsAddr, 7);

            var value = buffer[SecondsAddr];
            var halted = (value & 0x80) > 0; //CH bit is set
            if (halted)
                throw new InvalidOperationException("DS1307 halted");

            var second = value.FromBCD();
            var minute = buffer[MinutesAddr].FromBCD();

            value = buffer[HoursAddr];
            var mode12 = (value & 0x40) > 0; //12-hour mode
            if (mode12)
                throw new InvalidOperationException("DS1307 in 12-hour mode");

            var hour = value.FromBCD();
            var day = buffer[DateAddr].FromBCD();
            var month = buffer[MonthAddr].FromBCD();
            var year = 2000 + buffer[YearAddr].FromBCD();
        ...
        public byte[] ReadRam()
        {
            var buffer = ReadRam(RamAddr, RamLength);
            if(buffer[0] != OkMarker || buffer[1] != OkMarker)
                return new byte[0];

            var len = buffer[2];
            var result = Utility.ExtractRangeFromArray(buffer, 3, len);
            return result;
        }

        private byte ReadRam(byte address)
        {
            var result = ReadRam(address, 1);
            return result[0];
        }
        ...
        private byte[] ReadRam(byte address, int expectedBytes)
        {
            ...
            var actual = _device.Execute(transactions, Timeout);
            var expected = 1 + expectedBytes;
        ...
        private void WriteRam(byte address, params byte[] data)
        {
            ...
            var buffer = Utility.CombineArrays(new[] {address}, data);
        ...
        {
            var buffer = Reflection.Serialize(data, typeof(string));
            WriteRam(buffer);
        }
